...
merge_ERA5_FLUX <- function(filename_FLUX, filename_ERA5,
  ...
  merge_ERA5_FLUX <- function(filename_FLUX, filename_ERA5,
    ...
    #filename_ERA5 <- 'data_merge/US-EvM_ERA_2020_2023_half.csv'
    
    varname_FLUX <- c('P')
    varname_ERA5 <- c('tp')
    #varname_FLUX <- c('TA')# SW_IN, RH
    #varname_ERA5 <- c('t2m')  # 'ssrd', 'd2m', 't2m'
    ...
    dt <- as.numeric(difftime(ymd_hm(data_BASE$TIMESTAMP_END[2]), ymd_hm(data_BASE$TIMESTAMP_END[1]), s=c('hours')))
  ...
  data_ERA5_intp <- merge_ERA5_FLUX(filename_FLUX, filename_ERA5, varname_FLUX, varname_ERA5)
