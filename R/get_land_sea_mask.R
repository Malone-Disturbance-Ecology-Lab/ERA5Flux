...
#'
#' @description
#' This function downloads the land-sea mask from ERA5. The mask is needed in order to run `download_ERA5()`. The mask can be found online here:
...
#' If more than 60 seconds is needed to download the file, adjust the `timeout` parameter accordingly.
#'
...
#' @param timeout (numeric) Number of seconds until the download times out. Default is 60 seconds but can be set higher to allow for more time to download.
#'
...
get_land_sea_mask <- function(download_path = "lsm_1279l4_0.1x0.1.grb_v4_unpack.nc",
  ...
  # Default is 60 seconds but can be set for longer to allow sufficient time to download the file
